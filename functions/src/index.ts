...
function uid(ctx: any): string {
  ...
  if (!uid) throw new HttpsError("unauthenticated", "Sesi tidak terautentikasi.");
  return uid;
}
...
async function isPengawas(uid: string): Promise<boolean> {
...
export const ambilProfil = onCall(async (request) => {
  const u = uid(request);
...
export const buatUjian = onCall(async (request) => {
  const u = uid(request);
  if (!await isPengawas(u) && !await isAdmin(u)) {
    throw new HttpsError("permission-denied", "BUKAN_PENGAWAS");
  }
  ...
  if (!p_url || !/^https?:\/\//.test(p_url)) {
    throw new HttpsError("invalid-argument", "URL_TIDAK_VALID");
  }
  if (!p_durasi_menit || p_durasi_menit < 1 || p_durasi_menit > 600) {
    throw new HttpsError("invalid-argument", "DURASI_TIDAK_VALID");
  }

  let token = cleanToken(p_token || "");
  if (!token) token = genToken6();

  // Check uniqueness
  const existing = await db.collection("tokens").where("token", "==", token).limit(1).get();
  if (!existing.empty) {
    throw new HttpsError("already-exists", "TOKEN_BENTROK");
  }
  ...
  const docRef = await db.collection("tokens").add({
    ...
    expired_at: admin.firestore.Timestamp.fromDate(
      new Date(Date.now() + berlakuJam * 3600 * 1000)
    ),
...
export const daftarUjianAktif = onCall(async (request) => {
  const u = uid(request);
  if (!await isPengawas(u) && !await isAdmin(u)) {
    throw new HttpsError("permission-denied", "BUKAN_PENGAWAS");
  }
...
export const hapusUjian = onCall(async (request) => {
  const u = uid(request);
  if (!await isPengawas(u)) throw new HttpsError("permission-denied", "BUKAN_PENGAWAS");
  ...
  if (tokenSnap.empty) throw new HttpsError("not-found", "TIDAK_DITEMUKAN");
  ...
  for (const sess of sessSnap.docs) {
    ...
    for (const v of vSnap.docs) batch.delete(v.ref);
...
export const tutupUjianPengawas = onCall(async (request) => {
  const u = uid(request);
  if (!await isPengawas(u) && !await isAdmin(u)) {
    throw new HttpsError("permission-denied", "BUKAN_PENGAWAS");
  }
  ...
  if (tokenSnap.empty) throw new HttpsError("not-found", "TIDAK_ADA");
...
export const klaimSesi = onCall(async (request) => {
  const u = uid(request);
  ...
  if (!p_nama || p_nama.trim().length < 2) {
    throw new HttpsError("invalid-argument", "NAMA");
  }
  if (!p_device_hash || p_device_hash.length < 16) {
    throw new HttpsError("invalid-argument", "DEVICE");
  }

  const vBersih = cleanToken(p_token || "");
  if (vBersih.length < 4) throw new HttpsError("invalid-argument", "TOKEN_TIDAK_VALID");
  ...
  const rateSnap = await db.collection("percobaan_klaim")
    ...
    .get();
  if (rateSnap.size >= 10) {
    throw new HttpsError("resource-exhausted", "TERLALU_SERING");
  }
  ...
  if (tokenSnap.empty) {
    ...
    throw new HttpsError("not-found", "TOKEN_TIDAK_ADA");
  }
  ...
  if (now < mulaiMs) throw new HttpsError("failed-precondition", "BELUM_MULAI");
  if (now > expiredMs) throw new HttpsError("failed-precondition", "EXPIRED");
  ...
  if (!existSess.empty) {
    ...
    const sd = sess.data();
    if (sd.status === "dihentikan") throw new HttpsError("failed-precondition", "DIHENTIKAN");
    if (sd.status === "selesai") throw new HttpsError("failed-precondition", "SELESAI");
  ...
  if (tokenData.kunci_perangkat) {
    ...
    if (!devSnap.empty) throw new HttpsError("failed-precondition", "PERANGKAT_DIPAKAI");
  }

  // Quota check
  if (tokenData.max_peserta && tokenData.jumlah_klaim >= tokenData.max_peserta) {
    throw new HttpsError("resource-exhausted", "KUOTA_PENUH");
  }
...
export const heartbeat = onCall(async (request) => {
  ...
  if (!sessSnap.exists || sessSnap.data()!.uid !== u) {
    throw new HttpsError("not-found", "SESI_TIDAK_ADA");
  }
  ...
  if (sd.status === "dihentikan") return { ok: false, kode: "DIHENTIKAN" };
  if (sd.status === "selesai") return { ok: false, kode: "SELESAI" };
...
export const akhiriSesi = onCall(async (request) => {
  const u = uid(request);
  const { p_session_id } = request.data;
  const sessSnap = await db.collection("sessions").doc(p_session_id).get();
...
export const catatPelanggaran = onCall(async (request) => {
  const u = uid(request);
  ...
  const sd = sessSnap.data()!;
  if (sd.status !== "aktif") throw new HttpsError("failed-precondition", "SESI_TIDAK_AKTIF");
...
export const pantauKelas = onCall(async (request) => {
  const u = uid(request);
  if (!await isPengawas(u) && !await isAdmin(u)) {
    throw new HttpsError("permission-denied", "BUKAN_PENGAWAS");
  }
  ...
  const tokenSnap = await db.collection("tokens")
    .where("token", "==", cleanToken(p_token))
    .where("is_active", "==", true).limit(1).get();
  if (tokenSnap.empty) throw new HttpsError("not-found", "TOKEN_TIDAK_ADA");
...
export const detailPeserta = onCall(async (request) => {
  const u = uid(request);
  if (!await isPengawas(u) && !await isAdmin(u)) {
    throw new HttpsError("permission-denied", "BUKAN_PENGAWAS");
  }
  const { p_session_id } = request.data;
  const sessSnap = await db.collection("sessions").doc(p_session_id).get();
  if (!sessSnap.exists) throw new HttpsError("not-found", "SESI_TIDAK_ADA");
  const sd = sessSnap.data()!;
  const now = Date.now();
...
export const tindakanPengawas = onCall(async (request) => {
  const u = uid(request);
  if (!await isPengawas(u) && !await isAdmin(u)) {
    throw new HttpsError("permission-denied", "BUKAN_PENGAWAS");
  }
  ...
  if (p_aksi === "hentikan") {
    ...
    if (!p_nilai || p_nilai <= 0 || p_nilai > 180) {
      throw new HttpsError("invalid-argument", "NILAI_TIDAK_VALID");
    }
    const sd = sessSnap.data()!;
    ...
    throw new HttpsError("invalid-argument", "AKSI_TIDAK_DIKENAL");
  }
  return { ok: true };
});
...
export const ambilKelas = onCall(async (request) => {
  const u = uid(request);
  if (!await isPengawas(u) && !await isAdmin(u)) {
    throw new HttpsError("permission-denied", "TIDAK_AUTORIZASI");
  }
...
export const tambahKelas = onCall(async (request) => {
  const u = uid(request);
  if (!await isAdmin(u)) throw new HttpsError("permission-denied", "TIDAK_AUTORIZASI");
...
export const hapusKelas = onCall(async (request) => {
  const u = uid(request);
  if (!await isAdmin(u)) throw new HttpsError("permission-denied", "TIDAK_AUTORIZASI");
...
export const updateKelas = onCall(async (request) => {
  const u = uid(request);
  const { p_id, p_wali_kelas, p_jumlah_siswa } = request.data;
  const doc = await db.collection("kelas").doc(p_id).get();
  if (!doc.exists || doc.data()!.dibuat_oleh !== u) throw new HttpsError("permission-denied", "TIDAK_AUTORIZASI");
  const update: Record<string, any> = {};
  if (p_wali_kelas !== undefined) update.wali_kelas = p_wali_kelas;
  if (p_jumlah_siswa !== undefined) update.jumlah_siswa = p_jumlah_siswa;
...
export const daftarPengguna = onCall(async (request) => {
  const u = uid(request);
  if (!await isAdmin(u)) throw new HttpsError("permission-denied", "BUKAN_ADMIN");
  const snap = await db.collection("pengguna").orderBy("role").orderBy("nama_lengkap").get();
  const data = snap.docs.map(d => ({ id: d.id, ...d.data() }));
  return { ok: true, data };
});

export const tambahPengguna = onCall(async (request) => {
  const u = uid(request);
  if (!await isAdmin(u)) throw new HttpsError("permission-denied", "BUKAN_ADMIN");
  ...
  if (!userRecord) throw new HttpsError("not-found", "USER_TIDAK_DITEMUKAN");
...
export const hapusPengguna = onCall(async (request) => {
  const u = uid(request);
  if (!await isAdmin(u)) throw new HttpsError("permission-denied", "BUKAN_ADMIN");
  if (request.data.p_id === u) throw new HttpsError("failed-precondition", "TIDAK_BOLEH_HAPUS_DIRI");
  await db.collection("pengguna").doc(request.data.p_id).delete();
  return { ok: true };
});

export const updatePengguna = onCall(async (request) => {
  const u = uid(request);
  if (!await isAdmin(u)) throw new HttpsError("permission-denied", "BUKAN_ADMIN");
  const { p_id, p_nama_lengkap, p_role, p_mata_pelajaran } = request.data;
  const update: Record<string, any> = {};
  if (p_nama_lengkap !== undefined) update.nama_lengkap = p_nama_lengkap;
  if (p_role !== undefined) update.role = p_role;
  if (p_mata_pelajaran !== undefined) update.mata_pelajaran = p_mata_pelajaran;
...
export const jadikanPengawas = onCall(async (request) => {
  const u = uid(request);
  if (!await isAdmin(u) && !await isPengawas(u)) {
    throw new HttpsError("permission-denied", "DITOLAK");
  }
...
export const cabutPengawas = onCall(async (request) => {
  const u = uid(request);
  if (!await isAdmin(u)) throw new HttpsError("permission-denied", "HANYA_ADMIN");
  const { p_email } = request.data;
  const userRecord = await auth.getUserByEmail(p_email).catch(() => null);
  if (!userRecord) throw new HttpsError("not-found", "TIDAK_DITEMUKAN");
  ...
  if (adminSnap.size <= 1) {
    ...
    if (target) throw new HttpsError("failed-precondition", "INI_ADMIN_TERAKHIR");
  }
